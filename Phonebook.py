def work_with_phonebook(phonebook):
     while True:
        ...
        choice = input('1 - Копировать данные\n2 - Найти контакт\n3 - Добавить контакт\n\
4 - Изменить контакт\n5 - Удалить контакт\n6 - Просмотреть все контакты\n0 - Выйти из приложения\n')
        print()
        if choice == '1':
            file_to_add = input('Введите название копируемого файла: ')
            copy_data(file_to_add, phonebook)
        elif choice == '2':
...
def search_contact(contact_list: list):
    search_field, search_value = search_parameters()
    search_result = []
    for contact in contact_list:
...
def change_phone_number(file_name):
    ...
    number_to_change = search_contact(contact_list)
...
def copy_data(file_to_add, phonebook):
    try:
        with open(file_to_add, 'r', encoding='utf-8') as new_contacts, open(phonebook, 'a', encoding='utf-8') as file:
            ...
            if line_number <= len(contacts_to_add):
                file.write(contacts_to_add[line_number - 1])
